...
const signupSchema = z.object({
  ...
  company: z.string().min(2, 'Company / RIA Firm name is required'),
...
export const SignupPage: React.FC = () => {
  ...
  const onSubmit = (data: SignupFormValues) => {
    ...
    addToast('success', 'Account created successfully! Welcome to FundFit AI.');
    navigate('/dashboard');
  };
    ...
    <div className="max-w-md mx-auto px-4 py-16">
      <div className="p-8 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-black text-white">Create Enterprise Account</h1>
          <p className="text-xs text-slate-400">Start 14-day full trial with 10,000 Monte Carlo path execution.</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="email"
                {...register('email')}
                placeholder="alex@quantcapital.com"
                className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white placeholder-slate-500 focus:border-cyan-400 focus:outline-none"
              />
            </div>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1">Company / RIA Firm</label>
            <div className="relative">
              <Building className="w-4 h-4 text-slate-500 absolute left-3 top-3" />
              <input
                type="text"
                {...register('company')}
                placeholder="Quant Wealth Management"
                className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white placeholder-slate-500 focus:border-cyan-400 focus:outline-none"
              />
            </div>
            {errors.company && <span className="text-[11px] text-rose-400 mt-1 block">{errors.company.message}</span>}
          </div>
